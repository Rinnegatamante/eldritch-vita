...
void patch_fname(char *fname) {
	if (!strncmp(fname, "./", 2)) {
		sprintf(patched_fname, "ux0:data/Eldritch/%s", &fname[2]);
	} else {
		sprintf(patched_fname, "ux0:data/Eldritch/%s", fname);
	}
}
...
extern "C" int __real_sceIoMkdir (const char *fname, SceMode mode);
extern "C" int __wrap_sceIoMkdir(const char *fname, SceMode mode) {
	//printf("mkdir %s\n", fname);
	if (!strncmp(fname, "ux0:", 4)) {
		return __real_sceIoMkdir(fname, mode);
	} else {
		patch_fname((char *)fname);
	}
	return __real_sceIoMkdir(patched_fname, mode);
}
